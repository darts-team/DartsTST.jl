...
# osc_type = "USRP"
osc_type = "PERF" # near perfect oscillator
...
sync_pri    = 1 # sync repetition interval
...
@sync @distributed for ntrial = 1 : Ntrials
     ...
     for k = 1 : numCoeffs
        ...
        parameters.osc_coeffs = repeat(a_coeff_dB,Np) # put together all oscillator coeffs
        println("Starting oscillator coeff value: ", test_coeff)
...
# outputfilename_data = "syncModule_MonteCarlo_mode_$mode"*"_sync_osc_sweep_"*freq_text* "_imageData.jld2" # output filename for image data. doesn't save metrics
# @save outputfilename_data ideal_image_3D tomo_data osc_coeff_sweep s_loc_1 s_loc_2 s_loc_3

outputfilename = "syncModule_MonteCarlo_mode_$mode"*"_$osc_type"*"_coeff_number"*"$coeff_number"*"_sync_osc_sweep_"*freq_text* ".jld2" # this is the output filename that the data is saved to using JLD2
