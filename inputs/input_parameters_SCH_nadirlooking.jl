...
s_loc_1= (-10:.5:10) .+ t_loc_1  # deg latitude if LLH, along-track if SCH, X if XYZ
s_loc_2= (-40:2:40)  .+ t_loc_2 # deg longitude if LLH, cross-track if SCH, Y if XYZ
s_loc_3= (-15:.1:15)  .+ t_loc_3 # m  heights if LLH or SCH, Z if XYZ
...
PSF_image_point=1 # 1: peak location, 2: target location, 3: center of 3D scene, 4: peak location (upsample image)
# simulation options
