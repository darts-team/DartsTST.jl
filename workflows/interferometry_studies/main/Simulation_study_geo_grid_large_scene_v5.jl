...
platform_ref_point      = [34.4234;-118.96;697.5e3] # Mojave desert

platform_heading        = 0.0
platform_look_dir       = "right" 
platform_look_angle     = 20.0  

lat_res                 = 0.000033 #0.000075
lon_res                 = 0.000033
lat_extent              = 0.0123 #0.00333 #0.0075 #0.0123 * 2 #0.0108 #*2 #8 #2 #0.000333
lon_extent              = 0.02261  #0.0099 #0.02261 * 3 #*3#10 #3 #0.84 #0.02261*1 #0.05  #0.00165
NB_lat                  = 1
NB_lon                  = 1
...
Sim_idx                 = 202   # For output file reference
...
for B_idx = 1:length(trg_ref_lat_list)  
    ...
    @timeit to "Block time " begin
        ...
        #t_loc_3xN               = SimSetup.define_target_pixels(t_loc_1_range, t_loc_2_range, t_loc_3_range, DEM_region, num_targ_vol, target_mode, lat_res/2, lon_res/2, 0.0)
        ...
        t_ref_1xN               = ones(size(t_loc_3xN,2))
        ...
        t_rsf_tot           = 0.01*params.pulse_length # s duration of RSF #0.01
        ...
        global Nrsf             = Int32(length(t_rsf))
        global Δt_ft            = t_rx[2]-t_rx[1] # fast-time resolution
        global rawdata_const    = 2*pi/params.λ 
        global rawdata          = SharedArray(zeros(ComplexF32, Nst,Np,Nft) )
        global ref_delay        = 2*ref_range/c # reference delay
        global ind_offset       = Int32(floor(Nft/2)) + Int(-floor(Nrsf/2))
        ...
        output_data[:,:,13]         = reshape(t_targets_ref_corr,3, dims_s_2, dims_s_1)[1,:,:] # reflectivity of 1st target
