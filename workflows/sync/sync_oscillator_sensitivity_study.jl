...
params = UserParameters.inputParameters(sync_a_coeff_dB = a_coeff_dB,s_loc_1 = -40:2:40,PSF_image_point=1,PSF_cuts=1,display_tomograms=0,user_defined_orbit=0,include_antenna=false,fc = 3e9)
...
@sync @distributed for ntrial = 1 : Ntrials
     ...
     for k = 1 : numCoeffs
        ...
        params = UserParameters.inputParameters(sync_a_coeff_dB = a_coeff_dB,s_loc_1 = -40:2:40,PSF_image_point=1,PSF_cuts=1,display_tomograms=0,user_defined_orbit=0,
        include_antenna=false,no_sync_flag = true,enable_sync_phase_error=true,fc = 3e9)
...
# 
# # generate set of example parameters to get scene size
# params = UserParameters.inputParameters(sync_a_coeff_dB = a_coeff_dB,PSF_image_point=1,display_tomograms=0,user_defined_orbit=0,include_antenna=false)
# # Check consistency of input parameters
# paramsIsValid = UserParameters.validateInputParams(params)
# 
# image_3D = TomoWorkflow.generate_tomo(params)
...
# ## Initialize result vectors
# peaks       = SharedArray{Float64}(numCoeffs,Ntrials)
# resolutions = SharedArray{Float64}(numCoeffs,Ntrials)
# PSLRs       = SharedArray{Float64}(numCoeffs,Ntrials)
# ISLRs       = SharedArray{Float64}(numCoeffs,Ntrials)
# loc_errors  = SharedArray{Float64}(numCoeffs,Ntrials)
# tomo_data   = SharedArray{Float64}(Ns_1,Ns_2,Ns_3,numCoeffs,Ntrials)
# ## run trials
# @sync @distributed for ntrial = 1 : Ntrials
# 
...
# 
#         #TODO: do we want to use sync or not? 
#         params = UserParameters.inputParameters(sync_a_coeff_dB = a_coeff_dB,PSF_image_point=1,display_tomograms=0,user_defined_orbit=0,
#         include_antenna=false, no_sync_flag = true, enable_sync_phase_error=true)
#         #take tomographic cut
#         # params = UserParameters.inputParameters(sync_a_coeff_dB = a_coeff_dB,PSF_image_point=1,PSF_cuts=1,display_tomograms=0,user_defined_orbit=0,include_antenna=false)
#         # Check consistency of input parameters
#         paramsIsValid = UserParameters.validateInputParams(params)
