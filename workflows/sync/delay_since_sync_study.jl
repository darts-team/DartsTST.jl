...
params = UserParameters.inputParameters(s_loc_1=at_dim,PSF_image_point=1,PSF_cuts=2,display_tomograms=0,
...
# 
#             if use_meas_flag
#                 params = UserParameters.inputParameters(PSF_image_point=1, PSF_cuts=2, display_tomograms=0, user_defined_orbit=usr_orbit, s_loc_1 = at_dim, use_measured_psd_flag=use_meas_flag, mode=radar_mode,
#                 enable_sync_phase_error=true, delay_since_sync=delay, no_sync_flag=true, SAR_duration=sar_len, fc = center_freq,sync_f_osc=f_osc)
#             else
#                 params = UserParameters.inputParameters(PSF_image_point=1, PSF_cuts=2, display_tomograms=0, user_defined_orbit=usr_orbit, s_loc_1 = at_dim, sync_a_coeff_dB = coeffs,mode=radar_mode,
#                 enable_sync_phase_error=true, delay_since_sync=delay, no_sync_flag=true, SAR_duration=sar_len, fc = center_freq,sync_f_osc=f_osc)
#             end
# 
...
##TODO this is for a 1D measurement result..... using single axis (along n dimension typically)

## Initialize result vectors
peaks       = SharedArray{Float64}(numDelays,Ntrials)
resolutions = SharedArray{Float64}(numDelays,Ntrials)
PSLRs       = SharedArray{Float64}(numDelays,Ntrials)
ISLRs       = SharedArray{Float64}(numDelays,Ntrials)
loc_errors  = SharedArray{Float64}(numDelays,Ntrials)
tomo_data   = SharedArray{Float64}(params.Ns_1,params.Ns_2,params.Ns_3,numDelays,Ntrials)
...
@unpack mode, s_loc_1, s_loc_2, s_loc_3 = params
outputfilename = "syncModule_MonteCarlo_mode_$mode"*"_$sync_osc_type"*"_sync_DSS_sweep_along_n.jld2" # this is the output filename that the data is saved to using JLD2
# this saves the data into a JLD2 file. Data includes the estimates
@save outputfilename peaks resolutions PSLRs ISLRs ideal_res ideal_PSLR ideal_ISLR ideal_peak loc_errors delays s_loc_1 s_loc_2 s_loc_3 center_freq

outputfilename_data = "syncModule_MonteCarlo_mode_$mode"*"_$sync_osc_type"*"_sync_DSS_sweep_imageData_along_n.jld2" # output filename for image data. doesn't save metrics
@save outputfilename_data ideal_image_3D tomo_data delays s_loc_1 s_loc_2 s_loc_3
