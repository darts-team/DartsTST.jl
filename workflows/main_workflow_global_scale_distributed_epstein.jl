...
@sync @distributed for i1 = 1:size(lat_lon_idx,1)   
    ...
    if isnan(Canopy_heights[lat_lon_idx[i1,1],lat_lon_idx[i1,2],1])
        ...
        #=
        @timeit to "BPA using exisiting function " begin
...
#=             display(plot(plot_var_op, params.s_loc_3[41:81] ,xlabel="Vertical profile normalized radar intensity",
			...
			xtickfont=font(15), ytickfont=font(15), guidefont=font(15), titlefontsize=15)) =#
			#Correlation
			global Output_stat[lat_lon_idx[i1,1],lat_lon_idx[i1,2],2] = cor(transpose(plot_var_ip),plot_var_op)[1]
		end
		
		#print(Output_stat[lat_lon_idx[i1,1],lat_lon_idx[i1,2],2])
		#RMSE
		global Output_stat[lat_lon_idx[i1,1],lat_lon_idx[i1,2],1] =  sqrt( (sum( (transpose(plot_var_ip).-plot_var_op).^2 )) / length(targets_loc[3,:] ) )
        ...
        global Output_stat[lat_lon_idx[i1,1],lat_lon_idx[i1,2],3] = length(pks_ip)+1 #Total output peaks
		global Output_stat[lat_lon_idx[i1,1],lat_lon_idx[i1,2],4] = length(pks_op)+1 #Total output peaks
