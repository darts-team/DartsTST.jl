...
if display_geometry || display_RSF_rawdata || display_input_scene || display_tomograms != 0
    ...
    if display_RSF_rawdata; Plotting.plot_RSF_rawdata(ft, t_rx, MF, Srx, Np, Nst, rawdata, params); end
    if display_geometry
    ...
    if display_input_scene; Plotting.plot_input_scene(inputscene_3D, ts_coord_txt, params);end
    if display_tomograms != 0; Plotting.plot_tomogram(image_3D, ts_coord_txt, scene_axis11, scene_axis22, scene_axis33, PSF_metrics, params);end
    if display_input_scene; Plotting.plot_input_scene(diff_image3D, ts_coord_txt, params);end
end