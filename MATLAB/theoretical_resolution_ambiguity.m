...
alpha=60; % baseline tilt wrt horizontal (deg)
L=10e3:100:100e3; % aperture lengths (m)
dL=100:1:1000; % platform spacing (m)
res=5:0.1:50; % desired resolution (m)
d_amb=10:0.5:250; % desired nearest ambiguity location (m)
...
for i=1:3
    ...
    n_amb=wl*H./(cosd(theta)*p_amb(i)*dL*cosd(abs(theta-alpha))); % closest ambiguity location along perpendicular to look angle direction
...
ylabel('4 dB Resolution (along perpendicular to look angle) (m)')
...
ylabel('Ambiguity Location (along perpendicular to look angle) (km)')
legend('SAR','SIMO','MIMO')
set(gca,'fontsize',10)
...
for mode=1:3 %1: SAR, 2:SIMO, 3:MIMO
    ...
    figure;imagesc(d_amb,res,n_platform);colorbar;colormap jet
    title(['Minimum Number of Platforms Required (' modd ')'])
    xlabel('Nearest Ambiguity Location (m)')
    ylabel('4 dB Resolution (m)')
    set(gca,'fontsize',12)
