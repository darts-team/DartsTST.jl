...
t_θ=0.1-0.01:0.002:0.1+0.01 # deg latitude
t_ϕ=-0.01:0.002:0.01 # deg longitude
t_h=0:100:1000 # m  heights
...
scatter(t_xyz_grid[1,:],t_xyz_grid[2,:],t_xyz_grid[3,:],leg=false,camera=(30,30),markersize=0.1) #display grid in 3D
display(scatter!(p_xyz_grid[1,:],p_xyz_grid[2,:],p_xyz_grid[3,:],leg=false,camera=(30,30),markersize=1)) #display grid in 3D
display(scatter(p_xyz_grid[1,:],p_xyz_grid[2,:],p_xyz_grid[3,:],leg=false,camera=(30,30),markersize=3,xlim=(6878132-12000,6878132+12000),ylim=(-12000,12000),zlim=(-12000,12000))) #display grid in 3D
display(scatter(t_xyz_grid[1,:],t_xyz_grid[2,:],t_xyz_grid[3,:],leg=false,camera=(30,30),markersize=0.3,xlim=(6377100,6380100),ylim=(-1500,1500),zlim=(9000,12000))) #display grid in 3D
...
if mode==3
    ...
    display(plot(angle.(rawdata)*180/pi,leg=false,title="raw data phase (deg)"))
end

## DEFINE IMAGE SCENE

s_θ=0.1-0.012:0.0002:0.1+0.012 # deg latitude
s_ϕ=-0.012:0.0002:0.012 # deg longitude
s_h=-100:10:1100 # m  heights
...
display(heatmap(abs.(processed_image),c=cgrad([:black,:white]),xlabel="x",ylabel="y",title="z"))
#savefig("image.jpg")
