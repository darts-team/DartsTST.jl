...
#plot(orbit_time,orbit_pos[1,:,:]',xaxis=("time (sec)"),ylabel=("ECI X position (km)")) # plot the ECI orbit in the limited time range
#plot(orbit_time,orbit_pos[2,:,:]',xaxis=("time (sec)"),ylabel=("ECI Y position (km)")) # plot the ECI orbit in the limited time range
#plot(orbit_time,orbit_pos[3,:,:]',xaxis=("time (sec)"),ylabel=("ECI Z position (km)")) # plot the ECI orbit in the limited time range

# Add sync module code

include("inputs/input_parameters_sync.jl") # this is only here because it needs the number of platforms identified
# "parameters" is defined in "input_parameters_sync". We expect to change this to a single config file(?)
#   (or a default parameters file + another file)

# phase_err = Sync.get_sync_phase(slow_time,orbit_pos,parameters) # or is this supposed to be "orbit_pos_interp"?
phase_err = Sync.get_sync_phase(slow_time,orbit_pos_interp,parameters) # or is this supposed to be "orbit_pos_interp"?
display(plot(slow_time,phase_err'.*180/pi,xlabel=("Slow Time (s)"),ylabel=("Phase (deg)"), title=("RF Phase Error") ))
savefig("sync_phase_error.png")

Np=size(orbit_pos)[2] # number of platforms
Nst=size(slow_time)[1] # number of slow-time samples (pulses processed)
p_xyz=1e3*reshape(orbit_pos_interp,3,Np*Nst) # platform positions in xyz; for each platform, its position at each PRF treated as a different platform; output loops over platforms first, then slow-time
display(scatter(p_xyz[1,:],p_xyz[2,:],p_xyz[3,:],leg=false,camera=(20,40),markersize=3,xlabel="x (m)",ylabel="y (m)",zlabel="z (m)",title="Platform Positions at Each Pulse")) #display  position of each platform at each pulse in 3D
...
display(scatter!(p_xyz[1,:],p_xyz[2,:],p_xyz[3,:],leg=false,camera=(20,40),markersize=1,xlabel="x (m)",ylabel="y (m)",zlabel="z (m)",title="Platforms and Targets")) #display grid in 3D
#savefig("platforms_and_targets.png")
## GENERATE RAW DATA
#rawdata=Generate_Raw_Data.main(t_xyz_grid,p_xyz_grid,mode,tx_el,fc,a,e) # without RSF
ref_range=750e3 # reference range (ideally average of min and max ranges: TODO calculate from all target vs platform positions)
rawdata=Generate_Raw_Data.main_RSF(t_xyz_grid,p_xyz,mode,tx_el,fc,a,e,Srx,t_rx,ref_range) # with RSF #TODO do we need a and e? use structure as input
# plot raw data (RSF)
display(heatmap(t_rx,1:size(p_xyz)[2],20*log10.(abs.(rawdata)),c=cgrad([:black,:white]),xlabel="fast-time (s)",ylabel="TX/RX platform pairs",title="raw data amplitude (dB)"))
display(heatmap(t_rx,1:size(p_xyz)[2],angle.(rawdata)*180/pi,c=cgrad([:black,:white]),xlabel="fast-time (s)",ylabel="TX/RX platform pairs",title="raw data phase (deg)"))
## IMAGE SCENE
Ns_θ=length(s_θ)
Ns_ϕ=length(s_ϕ)
Ns_h=length(s_h)
